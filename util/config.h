...
struct Configure {
  uint32_t timeout_{20};  // timeout for propose operation.
  uint32_t pid_cookie_{0};

  uint32_t synod_id_{0};  // not used for the memoment, allocated globally for every synod, client use synod count to route request.
  ...
  uint32_t pg_type_{PGT_Synod3};  // must be configured accordingly with total_acceptor_
  uint32_t acceptor_worker_count_{5};
  uint32_t proposer_worker_count_{5};
  ...
  uint32_t plog_inst_num_{64};      // plog instance count for a synod, usally this should be relatively small, eg 1000?
